...
function generateETPPDF(jsonData) {
    ...
    const srpText = jsonData.sist_reg_preco ? "Sim, utilizará Sistema de Registro de Preços" : "Não utilizará Sistema de Registro de Preços";
    ...
    const mercadoText = `Pesquisa de Mercado: ${levMercado.pesquisa_mercado || 'Não informado'}\n\n` +
                       `Preço Médio Encontrado: R$ ${(levMercado.preco_medio || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})}\n\n` +
                       `Variação Percentual: ${levMercado.variacao_percentual || 0}%\n\n` +
                       `Fontes Consultadas: ${(levMercado.fontes || []).join(', ') || 'Não informado'}\n\n` +
    ...
    if (jsonData.quantidade_estimada && Object.keys(jsonData.quantidade_estimada).length > 0) {
        const quant = jsonData.quantidade_estimada;
        quantidadeText = `Total Estimado: R$ ${(quant.total_estimado || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})}\n\n` +
                        `Critérios de Dimensionamento: ${quant.criterios_dimensionamento || 'Não especificado'}`;
        
        if (quant.item_principal) {
            quantidadeText += `\n\nItem Principal:\n` +
                             `• Descrição: ${quant.item_principal.descricao || 'Não especificado'}\n` +
                             `• Quantidade: ${quant.item_principal.quantidade || 0}\n` +
                             `• Valor Unitário: R$ ${(quant.item_principal.valor_unitario || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;
        }
    }
    yPosition = addSection("9. QUANTIDADE ESTIMADA", quantidadeText, yPosition);

    // Seção 10 - JUSTIFICATIVA PARA NÃO PARCELAMENTO
    yPosition = addSection("10. JUSTIFICATIVA PARA NÃO PARCELAMENTO", jsonData.just_nao_parc, yPosition);

    // Seção 11 - VALOR TOTAL ESTIMADO
    yPosition = addSection("11. VALOR TOTAL ESTIMADO", jsonData.valor_total, yPosition);

    // Seção 12 - DEMONSTRAÇÃO DE RESULTADOS
    let resultadosText = 'Resultados não especificados';
    if (jsonData.demonst_resultados && Object.keys(jsonData.demonst_resultados).length > 0) {
        ...
        if (demo.indicadores_desempenho && demo.indicadores_desempenho.length > 0) {
            resultadosText += `INDICADORES DE DESEMPENHO:\n${demo.indicadores_desempenho.map(ind => `• ${ind}`).join('\n')}\n\n`;
        }
    ...
    yPosition = addSection("12. DEMONSTRAÇÃO DE RESULTADOS", resultadosText, yPosition);

    // Seção 13 - SERVIÇO CONTÍNUO
    const servicoContinuoText = jsonData.serv_continuo 
        ? `Sim, trata-se de serviço de natureza continuada.\n\nJustificativa: ${jsonData.justif_serv_continuo || 'Não informada'}`
        : 'Não se trata de serviço de natureza continuada.';
    yPosition = addSection("13. SERVIÇO DE NATUREZA CONTINUADA", servicoContinuoText, yPosition);
    ...
    yPosition = addSection("14. PROVIDÊNCIAS NECESSÁRIAS", providenciasText, yPosition);

    // Seção 15 - IMPACTOS AMBIENTAIS
    yPosition = addSection("15. IMPACTOS AMBIENTAIS", jsonData.impac_ambientais, yPosition);

    // Seção 16 - ALINHAMENTO COM PLS
    const plsText = jsonData.alinhamento_pls && jsonData.alinhamento_pls.length > 0 
        ? jsonData.alinhamento_pls.map(item => `• ${item}`).join('\n')
        : 'Nenhum critério específico de sustentabilidade definido';
    yPosition = addSection("16. ALINHAMENTO COM PLANO DE LOGÍSTICA SUSTENTÁVEL", plsText, yPosition);
    ...
    yPosition = addSection("18. EQUIPE DE PLANEJAMENTO", jsonData.equipe_de_planejamento, yPosition);
