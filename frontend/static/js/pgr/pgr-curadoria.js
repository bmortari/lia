document.addEventListener('DOMContentLoaded', function () {
    ...
    function inicializarSeletorPGR() {
        ...
        if (selector) {
            ...
            window.pgrData.forEach((pgr, index) => {
                ...
                option.value = index;
                option.textContent = `PGR ${pgr.id_pgr} - ${pgr.id_solucao ? `Solução ${pgr.id_solucao}` : 'Geral'}`;
    ...
    function carregarRiscosPorSolucao(riscoData) {
        ...
        if (!riscoData || !riscoData.id_solucao) {
            container.innerHTML = '<div class="text-center text-gray-500 p-4">Nenhum risco identificado</div>';
            return;
        }
    ...
    function extrairDadosAtuais() {
        ...
        const resumoAnalise = document.getElementById('resumo-analise')?.value?.trim() || '';
        const metodologiaAplicada = document.getElementById('metodologia-aplicada')?.value?.trim() || '';
        
        const solucaoMenorRisco = document.getElementById('solucao-menor-risco')?.value?.trim() || '';
        const solucaoMaiorRisco = document.getElementById('solucao-maior-risco')?.value?.trim() || '';
        const recomendacaoFinal = document.getElementById('recomendacao-final')?.value?.trim() || '';
